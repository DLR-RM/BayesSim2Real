...
# Project Imports
from src.probabilistic_inference import inference_utils
from src.probabilistic_inference.inference_core import ProbabilisticPredictor
from src.probabilistic_modeling.modeling_utils import covariance_output_to_cholesky, clamp_log_variance
...
class RetinaNetProbabilisticPredictor(ProbabilisticPredictor):
    ...
    def __init__(self, cfg):
        ...
        # Create transform
        self.dist_name_mutual_info = self.cfg.PROBABILISTIC_INFERENCE.DIST_NAME_MUTUAL_INFO # "bernoulli", "categorical"
        self.save_prob_vec_samples = self.cfg.PROBABILISTIC_INFERENCE.SAVE_PROB_VEC_SAMPLES
        self.only_epistemic_cov = self.cfg.PROBABILISTIC_INFERENCE.ONLY_EPISTEMIC_COV
        self.return_pen_emb = self.cfg.PROBABILISTIC_INFERENCE.RETURN_LOGITS
    ...
    def retinanet_probabilistic_inference(
        ...
        if is_epistemic:
            ...
            # cls_pen_embs = [output['cls_pen_embs'] for output in outputs_list]
            # reg_pen_emb = [output['reg_pen_emb'] for output in outputs_list]

            # Compute box classification and classification variance means
            def compute_means(vec):
                vec_mean = vec[-1]
                for i in range(len(box_cls) - 1):
                    vec_mean = [vec_mean[j] + vec[i][j] for j in range(len(vec_mean))]
                vec_mean = [vec_mean_i / len(vec) for vec_mean_i in vec_mean]
                return vec_mean

            box_cls_mean = compute_means(box_cls)
            outputs.update({'box_cls': box_cls_mean})
            if self.return_pen_emb:
                outputs.update({'cls_pen_embs': copy.deepcopy(box_cls_mean) })
                # outputs.update({'reg_pen_emb': compute_means(reg_pen_emb)})

            # Compute mutual information for box classification 
            logits_k = box_cls[-1]
            if self.save_prob_vec_samples:
                if self.dist_name_mutual_info == "bernoulli":
                    box_prob_samples = [Bernoulli(logits=box_cls[-1][j]).probs for j in range(len(logits_k))]
                    box_cls_avg_ent = [Bernoulli(logits=box_cls[-1][j]).entropy().sum(-1) for j in range(len(logits_k)) ] 
                    for i in range(len(box_cls) - 1):
                        for j in range(0, len(logits_k)):
                            box_prob_samples[j] = torch.cat((box_prob_samples[j], Bernoulli(logits=box_cls[i][j]).probs), dim=0)
                            box_cls_avg_ent[j] += Bernoulli(logits=box_cls[i][j]).entropy().sum(-1)

                    box_cls_avg_ent = [ box_cls_ent / len(box_cls) 
                                            for box_cls_ent in box_cls_avg_ent]

                    box_cls_mi = [Bernoulli(logits=mean_logits).entropy().sum(-1) - avg_ent 
                                        for mean_logits, avg_ent in zip(box_cls_mean, box_cls_avg_ent)]
                elif self.dist_name_mutual_info == "categorical": 
                    box_prob_samples = [Categorical(logits=box_cls[-1][j]).probs for j in range(len(logits_k))]
                    box_cls_avg_ent = [Categorical(logits=box_cls[-1][j]).entropy() for j in range(len(logits_k)) ] 
                    for i in range(len(box_cls) - 1):
                        for j in range(0, len(logits_k)):
                            box_prob_samples[j] = torch.cat((box_prob_samples[j], Categorical(logits=box_cls[i][j]).probs), dim=0)
                            box_cls_avg_ent[j] += Categorical(logits=box_cls[i][j]).entropy()

                    box_cls_avg_ent = [ box_cls_ent / len(box_cls) 
                                            for box_cls_ent in box_cls_avg_ent]

                    box_cls_mi = [Categorical(logits=mean_logits).entropy() - avg_ent 
                                        for mean_logits, avg_ent in zip(box_cls_mean, box_cls_avg_ent)]
                else:
            ...
            outputs.update({'box_cls_mi': box_cls_mi})
            if self.save_prob_vec_samples:
                for j in range(0, len(logits_k)):
                    box_prob_samples[j] = box_prob_samples[j].transpose(0, 1)
                outputs.update({'box_prob_samples': box_prob_samples})
        ...
        all_predicted_mi = []
        all_predicted_prob_vectors_samples = []
        all_cls_pen_embs = []
        # all_reg_pen_emb = []
        ...
        for i, anchors in enumerate(outputs['anchors']):
            ...
            if is_epistemic:
                box_cls_mi = outputs['box_cls_mi'][i][0]
            if self.save_prob_vec_samples:
                box_prob_samples = outputs['box_prob_samples'][i]
            if self.return_pen_emb:
                cls_pen_embs = outputs['cls_pen_embs'][i][0]
                # reg_pen_emb = outputs['reg_pen_emb'][i]
            ...
            if is_epistemic:
                box_cls_mi = box_cls_mi[anchor_idxs]
            if self.save_prob_vec_samples:
                box_prob_samples = box_prob_samples[anchor_idxs]
            if self.return_pen_emb:
                cls_pen_embs = cls_pen_embs[anchor_idxs]
                # reg_pen_emb = reg_pen_emb[anchor_idxs]
            ...
            all_predicted_prob_vectors.append(box_cls)
            all_classes_idxs.append(classes_idxs)
            all_predicted_boxes_epistemic_covar.append(box_reg_epistemic_covar)
            if is_epistemic:
                all_predicted_mi.append(box_cls_mi)
            if self.save_prob_vec_samples:
                all_predicted_prob_vectors_samples.append(box_prob_samples)
            if self.return_pen_emb:
                all_cls_pen_embs.append(cls_pen_embs)
                # all_reg_pen_emb.append(reg_pen_emb)
        ...
        return all_predicted_boxes, all_predicted_boxes_covariance, \
            cat(all_predicted_prob), cat(all_classes_idxs), \
            cat(all_predicted_prob_vectors), cat(all_predicted_mi) if is_epistemic else None, \
            cat(all_predicted_prob_vectors_samples) if self.save_prob_vec_samples else None, \
            cat(all_cls_pen_embs) if self.return_pen_emb else None
            # None # cat(all_reg_pen_emb) if self.return_pen_emb else None
    ...
    def post_processing_bayes_od(self, input_im):
        ...
        predicted_boxes, predicted_boxes_covariance, predicted_prob, classes_idxs, \
        predicted_prob_vectors, predicted_mi, predicted_prob_vectors_samples, \
        cls_pen_embs = outputs
        # cls_pen_embs, reg_pen_embs = outputs
        ...
        if len(predicted_boxes_list) > 0:
            if cls_merge_mode == 'bayesian_inference':
                ...
                if predicted_mi is not None:
                    predicted_mi = predicted_mi[keep]
                if predicted_prob_vectors_samples is not None:
                    predicted_prob_vectors_samples = predicted_prob_vectors_samples[keep]
                if cls_pen_embs is not None:
                    cls_pen_embs = cls_pen_embs[keep]
                    # reg_pen_embs = reg_pen_embs[keep]
            ...
            if predicted_prob_vectors_samples is not None:
                result.pred_cls_probs_samples = predicted_prob_vectors_samples
            if predicted_mi is not None:
                result.mutual_info = predicted_mi
            if cls_pen_embs is not None:
                result.cls_pen_embs = cls_pen_embs
                # result.reg_pen_embs = reg_pen_embs
        else:
