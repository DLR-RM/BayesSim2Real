...
class GeneralizedRcnnProbabilisticPredictor(ProbabilisticPredictor):
    ...
    def generalized_rcnn_probabilistic_inference(self,
        ...
        if is_epistemic:
            ...
            outputs = dict()
            outputs['proposals'] = outputs_list[0]['proposals']
            ...
            box_delta = torch.stack(box_delta_list, dim=0)
            box_cls = torch.stack(box_cls_list, dim=0)
            outputs['box_delta'] = box_delta
            outputs['box_cls'] = box_cls

            if box_reg_var_list[0] is not None:
                box_reg_var = torch.stack(box_reg_var_list, dim=0)
            else:
                box_reg_var = None
            outputs['box_reg_var'] = box_reg_var

            if box_cls_var_list[0] is not None:
                box_cls_var = torch.stack(box_cls_var_list, dim=0)
            else:
                box_cls_var = None
        ...
        if self.save_prob_vec_samples:
            outputs['box_prob_samples'] = Categorical(logits=box_cls).probs
        ...
        if outputs['box_cls_var'] is not None:
            ...
            box_cond_ent = Categorical(logits=box_cls).entropy()
            box_cond_ent = box_cond_ent.mean(0)
            box_cls = torch.nn.functional.softmax(box_cls, dim=-1)
            box_cls = box_cls.mean(0)
            box_mi = Categorical(probs=box_cls).entropy() - box_cond_ent
            outputs['box_cls_mi'] = box_mi

        # Remove background category
        scores = box_cls[:, :-1]
        num_bbox_reg_classes = box_delta.shape[2] // 4

        if is_epistemic:
            # Compute epistemic box covariance matrix
            box_delta_list = []
            for id_mcd in range(self.num_mc_dropout_runs):
                box_delta_i = box_delta[id_mcd, :, :].reshape(-1, num_bbox_reg_classes, 4) # Nxkx4
                samples_proposals = torch.repeat_interleave(
                    proposals.proposal_boxes.tensor.unsqueeze(1), num_bbox_reg_classes, dim=1) # Nxkx4
                box_delta_list_i = self.model.roi_heads.box_predictor.box2box_transform.apply_deltas(
                        box_delta_i.reshape(-1, 4), samples_proposals.reshape(-1, 4))
                box_delta_list.append(box_delta_list_i)
            box_delta_list = torch.stack(box_delta_list, dim=-1)
            _, box_reg_epistemic_covar = inference_utils.compute_mean_covariance_torch(box_delta_list)
            # epistemic_covar_mats.append(box_reg_epistemic_covar)
            epistemic_covar_mats = box_reg_epistemic_covar.reshape(-1, num_bbox_reg_classes, 4, 4) # torch.stack(epistemic_covar_mats, dim=0)
        box_delta = box_delta.mean(0)
        # num_bbox_reg_classes = box_delta.shape[2] // 4
        ...
        if outputs['box_reg_var'] is not None:
            ...
            if num_bbox_reg_classes == 1:
                ...
                epistemic_covar_mats = epistemic_covar_mats[filter_inds[:, 0], 0]
            else:
                box_reg_var = box_reg_var[filter_mask]
                epistemic_covar_mats = epistemic_covar_mats[filter_mask]
            ...
            if self.only_epistemic_cov:
                boxes_covars = epistemic_covar_mats
            else:
                boxes_covars += epistemic_covar_mats
        else:
            # predict boxes
            ...
            if is_epistemic:
                if num_bbox_reg_classes == 1:
                    boxes_covars = epistemic_covar_mats[filter_inds[:, 0], 0]
                else:
                    boxes_covars = epistemic_covar_mats[filter_mask]
            else: 
                boxes_covars = []

        return boxes, boxes_covars, scores, \
            filter_inds[:,1], box_cls[filter_inds[:, 0]],\
            outputs['box_cls_mi'][filter_inds[:, 0]] if is_epistemic else None, \
            outputs['box_prob_samples'][filter_inds[:, 0]] if self.save_prob_vec_samples else None
    ...
    def post_processing_bayes_od(self, input_im):
        ...
        predicted_boxes, predicted_boxes_covariance, predicted_prob, classes_idxs, \
            predicted_prob_vectors, predicted_mi, predicted_prob_vectors_samples = outputs
