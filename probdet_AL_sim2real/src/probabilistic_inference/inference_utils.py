...
# Project imports
from src.probabilistic_inference.probabilistic_retinanet_predictor import RetinaNetProbabilisticPredictor
from src.probabilistic_inference.probabilistic_rcnn_predictor import GeneralizedRcnnProbabilisticPredictor
...
def general_standard_nms_postprocessing(input_im,
    ...
    predicted_boxes, predicted_boxes_covariance, predicted_prob, classes_idxs, \
         predicted_prob_vectors, predicted_mi, predicted_prob_vectors_samples, predicted_cls_pen_embs = outputs
...
def bounding_box_bayesian_inference(cluster_means,
    ...
    for idx, cov in enumerate(cluster_covs):
        if np.linalg.det(cov) == 0:
            cluster_covs[idx] = cov + np.identity(cov.shape[0])*1e-5
...
def instances_to_json(instances, img_id, cat_mapping_dict=None, to_cpu_numpy=True):
    """
    Dump an "Instances" object to a COCO-format json that's used for evaluation.

    Args:
        instances (Instances): output of function probabilistic_detector_postprocess(),
    ...
    if not to_cpu_numpy:
        ...
        instances.set("image_id", torch.tensor([img_id]).repeat(len(instances)))
        return [instances]
    else:
        instances = instances[0]['instances']
        ...
        if hasattr(instances, "mutual_info"):
            mutual_info = instances.mutual_info.cpu().tolist()
        else:
            mutual_info = np.zeros_like(scores)
            
        if cat_mapping_dict is not None:
            classes = [
        ...
        if instances.has("pred_cls_probs_samples"):
            pred_cls_probs_samples = instances.pred_cls_probs_samples.cpu().tolist()
        else:
            pred_cls_probs_samples = None
        if instances.has("cls_pen_embs"):
            cls_pen_embs = instances.cls_pen_embs.cpu().tolist()
        else:
            cls_pen_embs = None
        ...
        for k in range(num_instance):
            if classes[k] != -1:
                result = {
                    ...
                    "bbox": boxes[k],
                    "score": scores[k],
                    "mutual_info": mutual_info[k],
                    "cls_prob": pred_cls_probs[k],
                    "bbox_covar": pred_boxes_covariance[k]
                }
                if pred_cls_probs_samples is not None:
                    result.update({"pred_cls_probs_samples": pred_cls_probs_samples[k]})
                if cls_pen_embs is not None:
                    result.update({"cls_pen_embs": cls_pen_embs[k]})
