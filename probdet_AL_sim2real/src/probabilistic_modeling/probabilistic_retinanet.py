...
class ProbabilisticRetinaNet(RetinaNet):
    ...
    def __init__(self, cfg):
        ...
        # freeze the whole backbone if MODEL.BACKBONE.FREEZE is true
        if cfg.MODEL.BACKBONE.FREEZE_ALL:
            for name_all, module_all in self._modules.items(): 
                if "backbone" in name_all:
                    # if "FPN" in name_all or "ResNet" in name_all:
                    for name, module in module_all._modules.items():  
                        logger.info(f"Freezing {name}")
                        for n, p in module.named_parameters():
                            logger.info(f" freezing {n}")
                            p.requires_grad = False
                else:
                    logger.info(f"Not freezing {name_all}")
        ...
        self.feature_shapes = [backbone_shape[f] for f in self.head_in_features]
    ...
    def forward(
        ...
        if num_mc_dropout_runs > 1:
            ...
            output_dict = self.produce_raw_output(anchors, features, return_pen_emb=return_pen_emb)
            return output_dict
    ...
    def produce_raw_output(self, anchors, features, return_pen_emb=False):
        """
        Given anchors and features, produces raw pre-nms output to be used for custom fusion operations.
        """
        # Perform inference run
        if return_pen_emb:
            pred_logits, pred_anchor_deltas, pred_logits_vars, pred_anchor_deltas_vars, cls_feats_list, reg_feats_list = self.head(
                features, return_pen_emb=return_pen_emb)
        else:
            pred_logits, pred_anchor_deltas, pred_logits_vars, pred_anchor_deltas_vars = self.head(
                features, return_pen_emb=return_pen_emb)
        ...
        if return_pen_emb:
            cls_feats = [
                permute_to_N_HWA_K(
                    x, self.feature_shapes[0].channels) for x in cls_feats_list]
            reg_feats = [
                permute_to_N_HWA_K(
                    x, self.feature_shapes[0].channels) for x in reg_feats_list]
        ...
        if return_pen_emb:
            raw_output.update({'cls_pen_emb': cls_feats,
                                'reg_pen_emb': reg_feats})
...
class ProbabilisticRetinaNetHead(RetinaNetHead):
    ...
    def forward(self, features, return_pen_emb=False):
        """
        Arguments:
        ...
        for feature in features:
            cls_feats = self.cls_subnet(feature)
            reg_feats = self.bbox_subnet(feature) 
            if return_pen_emb:
                cls_feats_list.append(cls_feats)
                reg_feats_list.append(reg_feats)
            logits.append(self.cls_score(cls_feats))
            bbox_reg.append(self.bbox_pred(reg_feats))
            if self.compute_cls_var:
                logits_var.append(self.cls_var(cls_feats))
            if self.compute_bbox_cov:
                bbox_cov.append(self.bbox_cov(reg_feats))
        ...
        if self.compute_bbox_cov:
            ...
            return_vector.append(None)
        if return_pen_emb:
            return_vector.extend([cls_feats_list, reg_feats_list])
