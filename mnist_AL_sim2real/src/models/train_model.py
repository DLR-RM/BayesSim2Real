...
from src.utils.ignite_utils import RestoringScoreGuard
from src.utils.ignite_utils import ignite_progress_bar
from src.utils.ignite_utils import (
    epoch_chain, chain, 
    chain_save_model, 
    log_epoch_results, 
    store_epoch_results, 
    store_iteration_results)
from src.models.sampler_model import SamplerModel, NoDropoutModel
...
def train_model(
    ...
    restoring_score_guard = RestoringScoreGuard(
    ...
    if early_stopping_patience < max_epochs:
        epoch_chain(trainer, validation_evaluator, validation_loader)
    if model_save_folder is not None:
        out_path = os.path.join(model_save_folder, "model_final.pth") # .format(trainer.current_epoch))
        chain_save_model(trainer, model, save_path=out_path)
