...
class BayesianModule(Module):
    ...
    def __init__(self, num_classes: int, pen_emb_dim):
        super().__init__()

        # TODO: use a different class for this!
        self.num_classes: int = num_classes
        self.pen_emb_dim = pen_emb_dim

    # Returns B x n x output
    def forward(self, input_B: torch.Tensor, k: int, return_pen_emb=False):
        BayesianModule.k = k

        input_B = self.deterministic_forward_impl(input_B)
        mc_input_BK = BayesianModule.mc_tensor(input_B, k)
        mc_output_BK, mc_pen_embd_BK = self.mc_forward_impl(mc_input_BK, return_pen_emb=return_pen_emb)
        mc_output_B_K = BayesianModule.unflatten_tensor(mc_output_BK, k)
        if return_pen_emb:
            mc_pen_embd_B_K = BayesianModule.unflatten_tensor(mc_pen_embd_BK, k)
            return mc_output_B_K, mc_pen_embd_B_K
        else:
