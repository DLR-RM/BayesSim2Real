import src.utils.laaos as laaos

import src.utils.acquisition_functions as acquisition_functions
import src.utils.acquisition_method as acquisition_method
import src.datasets.ds_functions as dataset_enum
...
def get_vip_args(store):
    ...
    lr = store.args.learning_rate
    nis = len(store.initial_samples)
    nap = nis + b * len(store.iterations)
    ss_ratio = store.args.initial_percentage
    tag = store.tag
    return VIPArgs(af, am, b, k, ds, nis, nap, tag, ss_ratio, lr)
